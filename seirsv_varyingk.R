...
NP = 3000; NMIF = 100; NUM_GUESSES = 40 # debug line
...
seir_step <- Csnippet("
  ...
  double mu_VS_eff;
  if (interval == 1) mu_VS_eff = 0;
  else mu_VS_eff = mu_VS;
  double dN_VS = rbinom(V, 1 - exp(-mu_VS_eff*dt));
...
init_params = c(b1=5,b2=10,b3=20,rho=.4, mu_EI=1/0.6, mu_IR=1/2.5, mu_RS = .01, 
                mu_SV = .002, mu_VS = .005,
                k1=2.5, k2=5, k3=1, eta=.1,N=KERALA_POP) 
...
covid_data |>
  ...
  pomp(
    ...
    paramnames=c("b1","b2","b3","mu_EI","mu_IR", "mu_RS", "mu_SV", "mu_VS", "eta","rho","k1", "k2", "k3", "N"),
    params=init_params,
...
step_size = rw_sd(b1 = .01, b2=.02, b3 = .02, mu_EI = .005, mu_IR = .005, 
              mu_RS = .00, mu_SV = 0, mu_VS = 0, rho = .002, k1 = .01, k2 = .01, k3 = .01, eta = ivp(.02))
  ...
  foreach(i=1:20,.combine=c,
    ...
    COVID_SEIR |>
      mif2(
        ...
        "mu_RS", "mu_SV", "mu_VS"),logit=c("rho","eta")),
        paramnames=c("b1","b2","b3", "k1", "k2", "k3", "mu_EI", "mu_IR", "mu_RS", "mu_SV", "mu_VS", "rho","eta")
      )
  } -> mifs_local
